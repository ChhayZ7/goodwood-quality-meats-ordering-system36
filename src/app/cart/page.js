...
// For true weight ranges (min !== max): returns a min/max range
// For single target weights (min === max): returns a single estimate
// For fixed price: returns exact total
function getLineEstimate(item) {
    if (item.product_type === 'FIXED') {
        const total = (item.price_cents * item.quantity) / 100
        return { min: total, max: total, isRange: false, isSingleTarget: false }
    }

    if (item.weight_option) {
        const pricePerKg = item.price_per_kg_cents
        const opt = item.weight_option
        const qty = item.quantity
        const isTrueRange = opt.max_weight_kg && opt.max_weight_kg !== opt.min_weight_kg

        if (isTrueRange) {
            const min = (pricePerKg * opt.min_weight_kg * qty) / 100
            const max = (pricePerKg * opt.max_weight_kg * qty) / 100
            return { min, max, isRange: true, isSingleTarget: false }
        } else {
            const estimate = (pricePerKg * opt.min_weight_kg * qty) / 100
            return { min: estimate, max: estimate, isRange: false, isSingleTarget: true }
        }
    }

    return { min: 0, max: 0, isRange: false, isSingleTarget: false }
}

// Format a line estimate for display
function formatEstimate(est) {
    if (est.isRange) return `$${est.min.toFixed(2)} — $${est.max.toFixed(2)}`
    if (est.isSingleTarget) return `~$${est.min.toFixed(2)}`
    return `$${est.min.toFixed(2)}`
}
...
export default function CartPage() {
    ...
    const hasWeightItems = items.some(item => item.product_type === 'WEIGHT_RANGE')
    const hasTrueRangeItems = items.some(item => {
        const opt = item.weight_option
        return item.product_type === 'WEIGHT_RANGE' && opt?.max_weight_kg && opt.max_weight_kg !== opt.min_weight_kg
    })
            ...
            <div className="max-w-3xl mx-auto px-6">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {item.weight_option && (
                                            <p className="text-xs mb-1" style={{ color: '#717182' }}>
                                                Weight: {item.weight_option.label}
                                            </p>
                                        )}
                ...
                <div className="bg-white rounded-lg p-6 mb-6" style={{ border: '1px solid #e5e5e5' }}>
                    ...
                    <div className="flex justify-between items-center pt-3 mt-3" style={{ borderTop: '1px solid #e5e5e5' }}>
                        <span className="text-sm" style={{ color: '#717182' }}>
                            {hasWeightItems ? 'Estimated Total' : 'Total'}
                        </span>
