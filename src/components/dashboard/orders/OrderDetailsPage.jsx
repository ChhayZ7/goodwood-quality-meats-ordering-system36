//Same as OrderListPage component
...
export default function OrderDetailPage({ role }) {
    ...
    return (
        <PageWrapper>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {lastStatusEntry && (
                            <p style={{ fontSize: '14px', color: '#555', margin: 0 }}>
                                ...
                                <span style={{ color: '#6B7280' }}>{lastStatusEntry.changed_by_user?.first_name} {lastStatusEntry.changed_by_user?.last_name} — {formatSmall(lastStatusEntry.created_at)}</span>
                            </p>
                        )}
                        {lastWeightEntry && (
                            <p style={{ fontSize: '14px', color: '#555', margin: 0 }}>
                                <strong style={{ color: '#1A1A1A' }}>Actual weights entered by:</strong>{' '}
                                <span style={{ color: '#6B7280' }}>{lastWeightEntry.changed_by_user?.first_name} {lastWeightEntry.changed_by_user?.last_name} — {formatSmall(lastWeightEntry.created_at)}</span>
                            </p>
                        )}
                        {auditLog.filter(e => e.field !== 'status' && e.field !== 'actual_weight').map((entry, i) => (
                            <p key={i} style={{ fontSize: '14px', color: '#555', margin: 0 }}>
                                ...
                                <span style={{ color: '#6B7280' }}>{entry.old_value ?? 'none'} → {entry.new_value} by {entry.changed_by_user?.first_name} {entry.changed_by_user?.last_name} — {formatSmall(entry.created_at)}</span>
                            </p>
                        ))}
                    </div>
                )}
            ...
            <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', padding: '24px', marginBottom: '20px' }}>
                        ...
                        <div key={item.id ?? i} style={{ display: 'grid', gridTemplateColumns: '1fr 140px 160px 100px 180px', padding: '14px 0', borderBottom: i < order.order_items.length - 1 ? '1px solid #F3F4F6' : 'none', alignItems: 'center' }}>
                            ...
                            {isWeightBased ? (
                                <input
                                    ...
                                    className="gw-input"
                                    style={{ width: '120px', padding: '8px 12px', fontSize: '13px', background: isLocked ? '#F9FAFB' : '#fff', cursor: isLocked ? 'not-allowed' : 'text', color: isLocked ? '#9CA3AF' : '#1A1A1A' }}
                                />
                            ) : (
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #E5E7EB' }}>
                    <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <span style={{ fontSize: '13px', color: '#6B7280' }}>Estimated Total: <strong style={{ color: '#1A1A1A' }}>{formatCents(order.total_cents)}</strong></span>
                        <span style={{ fontSize: '13px', color: '#6B7280' }}>Deposit Paid: <strong style={{ color: '#16A34A' }}>{formatCents(order.deposit_paid_cents)}</strong></span>
                    </div>
                </div>
            </div>
            ...
            {!isCancelled && (
                <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', padding: '24px', marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <select value={selectedStatus} onChange={e => setSelectedStatus(e.target.value)} className="gw-input" style={{ width: '220px', cursor: 'pointer' }}>
                            {ORDER_STATUSES.map(s => (
                                <option key={s} value={s}>{STATUS_CONFIG[s]?.label}</option>
                            ))}
                        </select>
                        <button onClick={handleUpdateStatus} disabled={saving || selectedStatus === order.status} className="btn-primary" style={{ padding: '10px 28px', fontSize: '14px', opacity: selectedStatus === order.status ? 0.5 : 1 }}>
            ...
            {/* Order Actions — cancel button is admin only */}
                ...
                {isAdmin && !isCancelled && showCancelForm && (
                    <div style={{ marginBottom: '24px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
                            <button onClick={handleCancel} disabled={cancelling} style={{ padding: '12px 28px', borderRadius: '10px', border: 'none', background: cancelling ? '#9CA3AF' : '#EF4444', color: '#fff', fontSize: '15px', fontWeight: 700, cursor: cancelling ? 'not-allowed' : 'pointer'}}>
                                {cancelling ? 'Cancelling…' : 'Confirm Cancellation'}
                            </button>
                            <button onClick={() => { setShowCancelForm(false); setCancelReason(''); setCancelReasonErr(false); setCancelError(null) }} style={{ padding: '12px 28px', borderRadius: '10px', border: '1.5px solid #E5E7EB', background: '#fff', color: '#1A1A1A', fontSize: '15px', fontWeight: 700, cursor: 'pointer' }}>
                                Go Back
                            </button>
                        </div>
                    </div>
